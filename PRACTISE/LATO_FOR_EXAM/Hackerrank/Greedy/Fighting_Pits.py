...
#
# this naive matchup requires all teams being sorted and get all time outs
def matchUp(team, x, y):
    ...
    while True:
        if team_x[x_attacker] >= y_attacker + 1:
            # y is wiped out
            winner = x
            break
        y_attacker = (y_attacker+1 ) -  team_x[x_attacker] - 1
        # y's turn to attack
        if team_y[y_attacker] >= x_attacker + 1:
            winner = y
            break
        x_attacker = (x_attacker+1 ) -  team_y[y_attacker] - 1
